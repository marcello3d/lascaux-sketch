...
describe('GotoMap.deserialize', () => {
  it('goto 2,0 4,0', () => {
    const { _gotoMap, _skipMap } = GotoMap.deserialize({
      gotos: [
        2, 0,
        4, 0
      ],
...
describe('GotoMap.computeRedo', () => {
  it('no gotos', () => {
    const map = makeGotoMap([])
    expect(map.computeRedo(0)).toBe(false)
    expect(map.computeRedo(1)).toBe(false)
    expect(map.computeRedo(2)).toBe(false)
  })
  it('one goto', () => {
    const map = makeGotoMap([
      // 0: draw (cursor 1)
    ...
    expect(() => map.computeRedo(1)).toThrowError('unexpected computeRedo from 1, expected 0')
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 0')
    expect(map.computeRedo(3)).toBe(false) // no redo
    expect(map.computeRedo(4)).toBe(false) // no redo
  })
  it('test two gotos', () => {
    const map = makeGotoMap([
      // 0: draw (cursor 1)
    ...
    expect(() => map.computeRedo(0)).toThrowError('unexpected computeRedo from 0, expected 4')
    expect(() => map.computeRedo(1)).toThrowError('unexpected computeRedo from 1, expected 4')
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 4')
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 4')
    expect(map.computeRedo(4)).toEqual(5)
    expect(() => map.computeRedo(5)).toThrowError('unexpected computeRedo from 5, expected 4')
    expect(map.computeRedo(6)).toBe(false) // no redo
    expect(map.computeRedo(7)).toBe(false) // no redo
  })
  it('test two adjacent gotos', () => {
    ...
    expect(() => map.computeRedo(0)).toThrowError('unexpected computeRedo from 0, expected 1')
    expect(map.computeRedo(1)).toBe(false)
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 1')
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 1')
    expect(map.computeRedo(4)).toBe(false)
    expect(map.computeRedo(5)).toBe(false)
  })
  it('test post redo', () => {
    const map = makeGotoMap([
      // 0: draw (cursor 1)
    ...
    expect(() => map.computeRedo(0)).toThrowError('unexpected computeRedo from 0, expected 2')
    expect(() => map.computeRedo(1)).toThrowError('unexpected computeRedo from 1, expected 2')
    expect(map.computeRedo(2)).toBe(false)
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 2')
    expect(map.computeRedo(4)).toBe(false)
    expect(map.computeRedo(5)).toBe(false)
  })
  it('test undo', () => {
    ...
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 1')
    expect(map.computeRedo(3)).toBe(false)
    expect(map.computeRedo(4)).toBe(false)
  })
  it('test undo-undo', () => {
    const map = makeGotoMap([
    ...
    expect(() => map.computeRedo(1)).toThrowError('unexpected computeRedo from 1, expected 0')
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 0')
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 0')
    expect(map.computeRedo(4)).toBe(false)
    expect(map.computeRedo(5)).toBe(false)
  })
  it('test undo-undo-redo', () => {
    ...
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 1')
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 1')
    expect(() => map.computeRedo(4)).toThrowError('unexpected computeRedo from 4, expected 1')
    expect(map.computeRedo(5)).toBe(false)
    expect(map.computeRedo(6)).toBe(false)
  })
  it('test post undo-redo-undo', () => {
    ...
    expect(() => map.computeRedo(2)).toThrowError('unexpected computeRedo from 2, expected 0')
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 0')
    expect(() => map.computeRedo(4)).toThrowError('unexpected computeRedo from 4, expected 0')
    expect(map.computeRedo(5)).toBe(false)
    expect(map.computeRedo(6)).toBe(false)
  })
  it('test post undo-redo-undo-redo', () => {
    ...
    expect(map.computeRedo(2)).toBe(false)
    expect(() => map.computeRedo(3)).toThrowError('unexpected computeRedo from 3, expected 2')
    expect(() => map.computeRedo(4)).toThrowError('unexpected computeRedo from 4, expected 2')
    expect(() => map.computeRedo(5)).toThrowError('unexpected computeRedo from 5, expected 2')
    expect(map.computeRedo(6)).toBe(false)
    expect(map.computeRedo(7)).toBe(false)
  })
...
describe('GotoMap.computeUndo', () => {
  it('no gotos, no keyframes', () => {
    ...
    expect(map.computeUndo(1)).toBe(false)
  })
  it('no gotos, 3 keyframes', () => {
    const map = makeGotoMap([])
    map.addKeyframe(0)
  ...
  it('1 keyframes with gotos', () => {
    ...
    expect(map.computeUndo(1)).toEqual(0)
    expect(map.computeUndo(2)).toEqual(0)
    expect(map.computeUndo(3)).toBe(false)
    expect(map.computeUndo(4)).toEqual(0)
    expect(map.computeUndo(5)).toEqual(0)
    expect(map.computeUndo(6)).toEqual(5)
    expect(map.computeUndo(7)).toEqual(5)
