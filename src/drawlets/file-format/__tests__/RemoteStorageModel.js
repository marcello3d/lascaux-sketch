...
describe('RemoteStorageModel.addStroke', () => {
  ...
  it('existing ranges', () => {
    let uploads = []
    const model = new RemoteStorageModel({
      getRangeMetadata: (callback) => callback(null, {
        ranges: [
          [0, 3],
          [4, 7]
        ]
  ...
  it('delayed existing ranges', () => {
    ...
    expect(() => model.addStroke('foo', 1, {})).toThrowError('cannot add stroke')
...
describe('RemoteStorageModel.getStroke', () => {
  it('get', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [[0, 3]] }))
    const downloadRange = jest.fn((start, end, callback) => callback(null, [
      {'payload': {}, 'time': 1, 'type': 'foo'},
      {'payload': {}, 'time': 2, 'type': 'foo'},
      {'payload': {}, 'time': 3, 'type': 'foo'},
      {'payload': {}, 'time': 4, 'type': 'foo'}
    ...
    expect(downloadRange.mock.calls.map(([start, end]) => [start, end])).toEqual([ [0, 3] ])
  })
  it('get multiple ranges', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [ [0, 0], [1, 1], [2, 2], [3, 3] ] }))
    const downloadRange = jest.fn((start, end, callback) => {
      switch (`${start}-${end}`) {
        case '0-0': return callback(null, [{'payload': {}, 'time': 1, 'type': 'foo'}])
        case '1-1': return callback(null, [{'payload': {}, 'time': 2, 'type': 'foo'}])
        case '2-2': return callback(null, [{'payload': {}, 'time': 3, 'type': 'foo'}])
        case '3-3': return callback(null, [{'payload': {}, 'time': 4, 'type': 'foo'}])
      }
    })
    const model = new RemoteStorageModel({ getRangeMetadata, downloadRange })
    expect(downloadRange.mock.calls).toEqual([])
    ...
    expect(await getStroke(model, 3)).toEqual({ type: 'foo', time: 4, payload: {} })
    expect(downloadRange.mock.calls.map(([start, end]) => [start, end])).toEqual([[0, 0], [1, 1], [2, 2], [3, 3]])
  })
  it('get multiple ranges 2', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [ [0, 1], [2, 5], [6, 10] ] }))
    const downloadRange = jest.fn((start, end, callback) => {
      switch (`${start}-${end}`) {
        case '0-1': return callback(null, [
          {'payload': {}, 'time': 1, 'type': 'foo'},
          {'payload': {}, 'time': 2, 'type': 'foo'}
        ])
        case '2-5': return callback(null, [
          {'payload': {}, 'time': 3, 'type': 'foo'},
          {'payload': {}, 'time': 4, 'type': 'foo'},
          {'payload': {}, 'time': 5, 'type': 'foo'},
          {'payload': {}, 'time': 6, 'type': 'foo'}
        ])
        case '6-10': return callback(null, [
          {'payload': {}, 'time': 7, 'type': 'foo'},
          {'payload': {}, 'time': 8, 'type': 'foo'},
    ...
    expect(await getStroke(model, 2)).toEqual({ type: 'foo', time: 3, payload: {} })
    expect(await getStroke(model, 3)).toEqual({ type: 'foo', time: 4, payload: {} })
    expect(await getStroke(model, 4)).toEqual({ type: 'foo', time: 5, payload: {} })
    expect(await getStroke(model, 5)).toEqual({ type: 'foo', time: 6, payload: {} })
    expect(await getStroke(model, 6)).toEqual({ type: 'foo', time: 7, payload: {} })
    expect(await getStroke(model, 7)).toEqual({ type: 'foo', time: 8, payload: {} })
    expect(await getStroke(model, 8)).toEqual({ type: 'foo', time: 9, payload: {} })
    expect(await getStroke(model, 9)).toEqual({ type: 'foo', time: 10, payload: {} })
    expect(await getStroke(model, 10)).toEqual({ type: 'foo', time: 11, payload: {} })
    expect(downloadRange.mock.calls.map(([start, end]) => [start, end])).toEqual([
      [0, 1],
      [2, 5],
      [6, 10]
    ])
  ...
  it('cache limit', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [ [0, 1], [2, 3] ] }))
    const downloadRange = jest.fn((start, end, callback) => {
      switch (`${start}-${end}`) {
        case '0-1': return callback(null, [
          {'payload': {}, 'time': 1, 'type': 'foo'},
        ...
        case '2-3': return callback(null, [
          {'payload': {}, 'time': 3, 'type': 'foo'},
          {'payload': {}, 'time': 4, 'type': 'foo'}
        ])
      }
    })
    const model = new RemoteStorageModel({ rangeCacheSize: 1, getRangeMetadata, downloadRange })
    expect(downloadRange.mock.calls).toEqual([])
    expect(await getStroke(model, 0)).toEqual({ type: 'foo', time: 1, payload: {} })
    expect(await getStroke(model, 1)).toEqual({ type: 'foo', time: 2, payload: {} })
    expect(await getStroke(model, 2)).toEqual({ type: 'foo', time: 3, payload: {} })
  ...
  it('get unavailable', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [] }))
  ...
  it('delayed get', async () => {
    ...
    const getRangeMetadata = jest.fn((callback) => { getRangeCallback = callback })
    const downloadRange = jest.fn((start, end, callback) => callback(null, [
      {'payload': {}, 'time': 1, 'type': 'foo'},
      {'payload': {}, 'time': 2, 'type': 'foo'},
      {'payload': {}, 'time': 3, 'type': 'foo'},
  ...
  it('get bad range', async () => {
    const getRangeMetadata = jest.fn((callback) => callback(null, { ranges: [[0, 1], [3, 4]] }))
    const model = new RemoteStorageModel({ getRangeMetadata })
    try {
